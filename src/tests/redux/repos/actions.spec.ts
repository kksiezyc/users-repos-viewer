...
import {SET_REPOS, SET_REPOS_ERROR, TOGGLE_REPOS_LOADING} from '../../../redux/repos/action-types';
...
describe('repos actions', (): void => {
    ...
    it('should fetch repos and dispatch proper methods for empty content', async (): Promise<
        ...
        const expectedActions = [
            {type: SET_REPOS_ERROR, reposError: ''},
            {type: TOGGLE_REPOS_LOADING, isReposLoading: true},
            {type: SET_REPOS_ERROR, reposError: 'Selected users does not have any repositories.'},
            {type: TOGGLE_REPOS_LOADING, isReposLoading: false},
        ];
        axios.CancelToken.source = jest.fn().mockImplementation(() => ({
            token: 'some value'
    ...
    it('should cancel previous request if ajaxRequest object is truthy', async (): Promise<void> => {
        const cancelTokenMock = {
            token: 'some value',
            cancel: jest.fn()
        }
        axios.CancelToken.source = jest.fn().mockImplementation(() => cancelTokenMock);
        await store.dispatch(fetchRepos('mock'));
        expect(cancelTokenMock.cancel).not.toHaveBeenCalled();
        await store.dispatch(fetchRepos('mock2'));
        expect(cancelTokenMock.cancel).toHaveBeenCalledWith(Requests.REQUEST_CANCEL);
    })
});
